const baseUrl = 'http://localhost:8080/v1/estacionamentos/';
...
function getEstacionamentos(callback) {
    getRequest(baseUrl, data => callback(JSON.parse(data)));
}

function getEstacionamentosByHour(id, timestamp, callback) {
    let url = baseUrl + id + '/findByHour?timestamp=' + timestamp;
    getRequest(url, data => callback(JSON.parse(data)));
}

function getEstacionamentosByDay(id, timestamp, callback) {
    let url = baseUrl + id + '/findByDay?timestamp=' + timestamp;
    getRequest(url, data => callback(JSON.parse(data)));
}

function getEstacionamentosByWeek(id, timestamp, callback) {
    let url = baseUrl + id + '/findByWeek?timestamp=' + timestamp;
    getRequest(url, data => callback(JSON.parse(data)));
}

function getEstacionamentosByMonth(id, timestamp, callback) {
    let url = baseUrl + id + '/findByMonth?timestamp=' + timestamp;
    getRequest(url, data => callback(JSON.parse(data)));
}

function getEstacionamentosByYear(id, timestamp, callback) {
